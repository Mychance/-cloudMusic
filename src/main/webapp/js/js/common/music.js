...
define(function (require, exports, module) {
    ...
    Player.prototype.init = function (data_id) {   // data.src, data.name, data.master
        var self = this;
        $.get('index/getMInfo/' + data_id, function (json) {
    ...
    Player.prototype._bind = function() {
        ...
        $(this.audio).on('loadstart', function() {     // 正在加载 loading
            ...
            var musicId = self.audio.musicId;
            $.get('index/getMInfo/'+musicId, function(json) {
                
                $('.play-ing .pbar .cur .cur-inner').width(0);
                $('.play-ing .ptitle a.title').html(json.name);
                $('.play-ing .ptitle a.singer').html(json.singerName);
                $(self.audio).attr('data-id', json.musicId);
                $('.play-head a').attr('href', './result.html?id=' + json.musicId);
                $('.play-head').attr('musicId', json.musicId);
                $('.play-ing .ptitle a').attr('href', './result.html?id=' + json.musicId);
