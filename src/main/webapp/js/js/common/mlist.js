...
define( function ( require, exports, module ) {
	...
	function MList() {
		...
		this.operCOL = '.fix-play .play-oper .icon-colle';	//收藏按钮
		this.operSHARE = '.fix-play .play-oper .icon-share'	;//收藏按钮			
	}
	...
	MList.prototype = {
		...
		_bindUI: function() {
			...
			$('.fix-bottom').on('mousedown', this.divList, function () {
			...
			}).on( 'click', this.btnColle, function() {
				...
				if ( !cookie('unique') ) {
					...
					$('#collection_form .j-flag ul').attr("multi",true);
				...
				$( self.divList ).hide();
			}).on( 'click', this.operCOL, function() {

				if ( !cookie('unique') ) {
					alert('您尚未登录');

				} else {
					var musicId = $('.play-head').attr('musicId');

					
					$('#collection_form .j-flag ul').attr("musicId",musicId);
					$.get("index/getLists?creatorId="+cookie('unique'),function(lists){
						$("#collection_form .j-flag ul").html('');
						$.each(lists,function(index,list){
				...
				dblclick : function() {
					$.get('index/getMInfo/' + $(this).attr('data-id'), function(json) {
						$('audio')[0].src = json.src;
						$('audio')[0].musicId = json.musicId;
						$('audio')[0].play();
					});
				}
				...
				self._update( 'remove' );
//			}).on('click',this.liDWN,function(){
//				if ( !cookie('unique') ) {
//					alert('您尚未登录');
//
//				} else {
//					var musicId = $(this).parents('li').attr('data-id');
//
//					
//					$.get("index/download?musicId="+musicId,function(lists){
//						
//					});
//				};
			}).on({
			...
			$('audio').on('canplay', function() {
				var isset = false,
				musicId = $('audio')[0].musicId;
				$.get('index/getMInfo/' + musicId , function(json) {
						dataID = json.musicId;
					...
					if (!isset) {
						...
						var html = '<li data-id="' + json.musicId + '">' +
							...
							'<a href="index/download/'+json.src+'" class="icn-dwn" title="下载"></a>' +
							'<a href="javascript:;" class="icn-del" title="删除"></a>' +
							'</div>' +
							'<div class="col col-3">' + json.singerName + '</div>' +
